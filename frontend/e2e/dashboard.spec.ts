...
test("home renders the unified PB client book and dashboard", async ({ page }) => {
  ...
  await expect(page.locator("[data-testid='client-select-client-001']")).toBeVisible();
  await expect(page.locator("[data-testid='client-select-client-002']")).toBeVisible();
  await expect(page.locator("[data-testid='dashboard-home']")).toBeVisible();
  await expect(page.locator("h1")).toContainText("고객장부");
  await expect(page.locator("[data-testid='dashboard-home']")).toContainText(
    "고객 A 요약 대시보드",
  );
  await expect(page.locator("[data-testid='client-monitoring-signals']")).toBeVisible();
});
...
test("selecting a client changes dashboard data without leaving home", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/\?client=client-002$/);
  await expect(page.locator("[data-testid='dashboard-home']")).toContainText(
    "고객 B 요약 대시보드",
  );
